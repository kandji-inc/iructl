class Iructl < Formula
  ...
  test do
    assert_match "iructl, version #{version}", shell_output("#{bin}/iructl --version")
    system bin/"iructl", "new", testpath/"test-repo"
    system "git", "-C", testpath/"test-repo", "rev-parse", "--is-inside-work-tree"
    system bin/"iructl", "profile", "new", "--output", testpath/"test-repo/profiles", "--name", "test-profile"
    system bin/"iructl", "profile", "list", "--repo", testpath/"test-repo", "--local"
    system bin/"iructl", "script", "new", "--output", testpath/"test-repo/scripts", "--name", "test-script"
    system bin/"iructl", "script", "list", "--repo", testpath/"test-repo", "--local"
  end
end
